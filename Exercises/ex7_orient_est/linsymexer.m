...
dxf= filter2(gy,filter2(dx,inim));  
dyf= filter2(gx,filter2(dy,inim));  
LS= (dxf + 1i*dyf).^2;   
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
i11= filter2(gy2,filter2(gx2,abs(LS))); %%average the magnitude of the orientation tensor with large gaussian
%%%%%%%%%%%%%%%%%%%%%%%

%display the results
LS(:,:,1)=mod(angle(i20),2*PI); % LS(:,:,1) is  2*angle where angle is the inclination angle of the fit orientation wrt which a linear symmetry exists.
LS(:,:,2)=abs(i20);%The second component LS(:,:,2) is Lambda_{worst} -Lambda_{best} that represents the difference of the total errors when 
                   %worst and best orientations are fit to the local neighborhood.
LS(:,:,3)=i11;%The third component LS(:,:,3) is the sum  of the best and worst total errors, Lambda_{worst}+Lambda_{best}.
